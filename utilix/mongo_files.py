    ...
    def __init__(self,
        ...
        if collection is None:
            if not readonly:
                ...
                mongo_url = utilix.uconfig.get('rundb_admin', 'mongo_rdb_url')
                mongo_user = utilix.uconfig.get('rundb_admin', 'mongo_rdb_username')
                mongo_password = utilix.uconfig.get('rundb_admin', 'mongo_rdb_password')
            else:
...
class DownloadAPI(MongoDownloader):
    def __init__(self,
                 readonly=True,
                 config_identifier='config_name',
                 ):
    ...
    def config_exists(self, config):
        ...
        """
        query = self.get_query_config(config)
        return self.DB.count_documents(query) > 0
    ...
    def get_gridfs_object(self,
        ...
        if self.config_exists(config_name):
            ...
            try:
                # This could return multiple since we upload files if
                # they have changed again! Therefore just take the last.
                fs_object = self.DB.get_last_version(**query)
                return fs_object
            except gridfs.NoFile as e:
                raise CouldNotLoadError(
                    f'{config_name} cannot be downloaded from GridFs') from e
    ...
    def open_response(self, fs_object):
        return fs_object.content

    def test_find(self):
        """Only inplemented for non API"""
        raise NotImplementedError('test_find not implemented')


class UploadAPI(MongoUploader):
    def __init__(self,
                 readonly=True,
                 config_identifier='config_name',
                 ):
    ...
    def config_exists(self, config):
        ...
        """
        query = self.get_query_config(config)
        return self.DB.count_documents(query) is not None
